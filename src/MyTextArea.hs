...
import Control.Lens ((&), (^.), element, _Just, non, (+~), (^?!), (?~), (^?), (.~), (%~), (<>~), ALens', ix, view)
...
instance CmbSyntax (TextAreaCfg s e) where
  syntax list = def {
    _tacSyntax = Just list
  }

instance CmbShowLineNumbers (TextAreaCfg s e) where
  showLineNumbers_ bool = def {
    _tacShowLineNumbers = Just bool
  }

instance CmbCurrentLineColor (TextAreaCfg s e) where
  ...
  handleEvent wenv node target evt = case evt of
    ButtonAction point btn BtnPressed clicks
      | clicks == 1 -> Just result where
        newPos = findClosestGlyphPos config state (localPoint point)
        newState = state {
    ...
    ButtonAction point btn BtnReleased clicks
      | clicks == 2 -> result where
        (tx, ty) = findClosestGlyphPos config state (localPoint point)
...
findClosestGlyphPos :: TextAreaCfg s e -> TextAreaState -> Point -> (Int, Int)
findClosestGlyphPos config state point = (newPos, lineIdx) where
  --Point x y = point
  Point x' y = point
  lineNumberWidth = if fromMaybe False (_tacShowLineNumbers config)
                    then 50
                    else 0
  x = if fromMaybe False (_tacShowLineNumbers config)
    then do
    let x'' = x' - lineNumberWidth
  ...
  -- newPos = do
  --   let val = maybe 0 fst (Seq.lookup 0 diffs)
  --   let max' = Seq.length lineGlyphs
  --   if (val-8) < 0
  --     then 0
  --     else do
  --     if val < max' - 8
